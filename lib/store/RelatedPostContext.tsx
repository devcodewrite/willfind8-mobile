...
interface PostContextType {
  ...
  getPostById: (id: number) => Post | undefined; // Selector function
}
...
export const PostProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchPosts = async (params?: {
    ...
    const { page, lastFetchFailed, loading } = postState;

    if (loading || lastFetchFailed) {
      console.log("Skipping fetch due to ongoing request or previous failure.");
      return;
    }
  ...
  const getPostById = (id: number): Post | undefined => {
    return postState.posts.find((post) => post.id === id);
  };
