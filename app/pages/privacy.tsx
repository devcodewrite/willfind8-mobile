...
export default function PageLayout() {
  ...
  const page = `
    ...
    </head><body><h3>${privacy?.title}</h3>${privacy?.content || ""}</body></html>`;

  useEffect(() => {
    fetchPageBySlug("privacy");
  }, [fetchPageBySlug]);
  return (
    <View style={{ flex: 1 }}>
      <Stack.Screen options={{ title: privacy?.title || "Privacy" }} />
      {loading ? (
        <View style={styles.container}>
          <ActivityIndicator size={"large"} animating />
        </View>
      ) : (
        <WebView containerStyle={{ flex: 1 }} source={{ html: page }} />
      )}
    </View>
  );
}
