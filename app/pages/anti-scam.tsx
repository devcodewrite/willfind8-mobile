...
export default function PageLayout() {
  ...
  const antiScam = pages.get("anti-scam");
  ...
  const page = `<!DOCTYPE html>
    ...
    </head><body><h3>${antiScam?.title}</h3>${antiScam?.content || ""}</body></html>`;

  useEffect(() => {
    fetchPageBySlug("anti-scam");
  }, [fetchPageBySlug]);

  return (
    <View style={{ flex: 1 }}>
      <Stack.Screen options={{ title: antiScam?.title || "Terms" }} />
      {loading ? (
